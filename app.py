...
# ── 3. DATA & MODEL LOADING FUNCTIONS ───────────────────────────────────────
@st.cache_data
def load_raw_data():
...
def load_ml_assets():
    ...
    if not base_path.exists():
        return None, None
    try:
        model = joblib.load(base_path / "gradient_boosting.pkl")
        with open(base_path / "feature_cols.json", "r") as f:
...
with tab_overview:
    if filtered_df.empty:
        ...
        st.write("")  # Using clean native write function for paragraph break spacing
...
with tab_prediction:
    ...
    if model is None:
        st.error("⚠️ The Machine Learning assets were not found in the `models/` directory. Deploy models via train_models.py first.")
    else:
