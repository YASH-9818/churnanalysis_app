...
# ── 3. DATA & MODEL LOADING FUNCTIONS WITH AUTO-TRAINING ───────────────────
@st.cache_data
def load_raw_data():
...
def load_ml_assets():
    ...
    pkl_path = base_path / "gradient_boosting.pkl"
    json_path = base_path / "feature_cols.json"
    
    # If files are missing, run training script on the cloud machine directly
    ...
    try:
        ...
        # If there's a version/OS loading mismatch error, re-train clean files right here
        if Path("train_models.py").exists():
            subprocess.run(["python", "train_models.py"])
            try:
                model = joblib.load(pkl_path)
                with open(json_path, "r") as f:
...
with st.sidebar:
    ...
    selected_geos = st.multiselect("Target Regions", options=all_geos, default=all_geos)
    
    all_genders = sorted(list(df_raw["Gender"].unique())) if not df_raw.empty else ["Male", "Female"]
    selected_genders = st.multiselect("Gender Segments", options=all_genders, default=all_genders)
    
    min_age = int(df_raw["Age"].min()) if not df_raw.empty else 18
    max_age = int(df_raw["Age"].max()) if not df_raw.empty else 100
    selected_age_range = st.slider("Age Parameters", min_age, max_age, (min_age, max_age))
...
st.markdown("An advanced ensemble machine learning system for portfolio attrition forecasting.")
st.markdown("---")
...
with tab_prediction:
    ...
    if model is None:
        st.error("⚠️ The Machine Learning models could not be loaded or trained. Check that train_models.py and European_Bank.csv are in the main folder.")
    else:
        st.markdown("Input the specific customer parameters below to calculate real-time churn liability scores.")
        
        p1, p2, p3 = st.columns(3)
        with p1:
            inp_geo = st.selectbox("Customer Primary Region", ["France", "Germany", "Spain"])
            inp_gender = st.selectbox("Customer Gender Identity", ["Male", "Female"])
            inp_age = st.number_input("Customer Current Age", min_value=18, max_value=100, value=35)
        with p2:
            inp_credit = st.slider("Credit Score Matrix", 300, 850, 650)
            inp_tenure = st.slider("Tenure Framework (Years)", 0, 10, 5)
            inp_products = st.number_input("Active Bank Products Used", min_value=1, max_value=4, value=2)
        with p3:
            inp_balance = st.number_input("Current Book Balance ($)", min_value=0.0, value=50000.0)
            inp_salary = st.number_input("Estimated Household Salary ($)", min_value=0.0, value=75000.0)
            inp_card = st.checkbox("Active Credit Card Holder", value=True)
            inp_member = st.checkbox("Engaged Active Member Status", value=True)
            
        if st.button("Execute Real-Time Scoring Pipeline", type="primary"):
            raw_input = {
...
with tab_simulator:
    ...
    st.markdown("Simulate systemic market pricing modifications or demographic strategy adjustments to gauge global performance shifts.")
    ...
    if model is None or df_raw.empty:
        ...
        s1, s2 = st.columns(2)
        with s1:
            mod_salary = st.slider("Global Estimated Salary Shift (%)", -50, 50, 0, step=5)
            mod_balance = st.slider("Global Account Balance Adjustment (%)", -50, 50, 0, step=5)
        with s2:
            force_active = st.selectbox("Simulate Global Member Engagement Campaign", ["No Change", "Convert All to Active", "Convert All to Inactive"])
            
        if st.button("Execute Batch Population Re-Scoring", type="secondary"):
            sim_df = df_raw.copy()
